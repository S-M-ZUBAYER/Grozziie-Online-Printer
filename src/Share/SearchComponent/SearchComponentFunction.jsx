...
export function filterDataBySearchFields(customersData, searchFields) {
    ...
    return customersData.filter(customer => {
        ...
        if (searchFields.isActiveRecipientAddress && searchFields.RecipientAddress) {
            isMatch = isMatch && customer?.receiver_address?.toLowerCase()?.includes(searchFields.RecipientAddress.toLowerCase());
        }

        // Check if isActiveAccountName is true and AccountName matches
        if (searchFields.isActiveAccountName && searchFields.AccountName) {
            isMatch = isMatch && customer?.account_name?.toLowerCase()?.includes(searchFields.AccountName.toLowerCase());
        }

        // Check if isActiveOrderId is true and OrderId matches
        if (searchFields.isActiveOrderId && searchFields.OrderId) {
            isMatch = isMatch && customer?.id?.toString()?.toLowerCase()?.includes(searchFields.OrderId.toLowerCase());
        }

        // Check if isActiveAmount is true and Amount matches
        if (searchFields.isActiveAmount && searchFields.Amount) {
            isMatch = isMatch && customer?.item_list[0]?.goods_count?.toString()?.toLowerCase()?.includes(searchFields.Amount.toLowerCase());
        }

        // Check if isActiveProduct is true and Product matches
        if (searchFields.isActiveProduct && searchFields.Product) {
            isMatch = isMatch && customer?.item_list[0]?.goods_name?.toLowerCase()?.includes(searchFields.Product.toLowerCase());
        }

        return isMatch;
    });
}
export function filterDataBySearchFieldsBatchPrint(customersData, searchFields) {
    return customersData.filter(customer => {
        let isMatch = true;
        // Check if isActiveRecipientAddress is true and RecipientAddress matches
        if (searchFields.isActiveRecipientAddress && searchFields.RecipientAddress) {
        ...
        // Check if isActiveAccountName is true and AccountName matches
        if (searchFields.isActiveAccountName && searchFields.AccountName) {
            isMatch = isMatch && customer?.receiver_name_mask?.toLowerCase().includes(searchFields.AccountName.toLowerCase());
        }
        ...
        // Check if isActiveProduct is true and Product matches
        if (searchFields.isActiveProduct && searchFields.Product) {
            isMatch = isMatch && customer?.item_list[0]?.goods_spec.toLowerCase().includes(searchFields.Product.toLowerCase());
        }
