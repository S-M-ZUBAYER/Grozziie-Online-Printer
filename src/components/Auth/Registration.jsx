...
const Registration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Make API call to register user
      const response = await fetch(
        "https://grozziieget.zjweiting.com:3091/CustomerService-Chat/api/v1/user/signup",
        {
          method: "POST",
          headers: {
            accept: "*/*",
        ...
        <div className="col-span-4 bg-white flex flex-col items-center py-8 rounded-r-2xl">
          ...
          <form onSubmit={handleSubmit} className="w-full px-10">
            ...
            <div className="mb-4">
              ...
              {error && (
                <p className="text-xs pt-1 text-red-500 font-bold">{error}</p>
              )}
            </div>
            ...
            <div className="flex items-center justify-center mb-5">
              <button
                className="bg-[#004368] hover:bg-opacity-60 text-white hover:text-black w-[150px] h-10 px-2 py-2 rounded-md cursor-pointer text-center mr-3 mt-2"
                type="submit"
                disabled={loading}
              >
                {loading ? (
